...
export default function AddPaymentDialog({ open, onOpenChange, invoices, onPaymentRecorded }: AddPaymentDialogProps) {
  ...
  useEffect(() => {
    if (!open || !currencyCode || !invoice || !selectedInvoiceId) return;
    ...
    fetchExchangeRate();
  }, [currencyCode, open, invoice, selectedInvoiceId, toast]);
  ...
  const fetchInvoiceDetails = async () => {
    ...
    setLoading(true);
    try {
      const data = await invoicesRepo.getInvoiceDetails(selectedInvoiceId);
      setInvoice(data);
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await invoicesRepo.recordPayment(selectedInvoiceId, {
        paid_amount: paidAmt,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          {/* Invoice Details */}
          {loading && selectedInvoiceId && !invoice ? (
            ...
            <DialogFooter className="pt-2">
              ...
              <Button 
                ...
                disabled={
                  ...
                  !selectedInvoiceId || 
                  !invoice || 
                  (invoice && (parseFloat(String(invoice.total_amount || 0)) - parseFloat(String(invoice.amount_paid || 0)) <= 0)) ||
                  !exchangeRate || 
                  exchangeRate <= 0 || 
                  fetchingRate
                }
